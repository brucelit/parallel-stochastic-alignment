...
# class Coordinator:
#     def __init__(self):
#         target_num = [1000000 for i in range(4)]
#         worker_handles = []
#
#     def register_worker(worker_handle):
#         worker_handles.append(worker_handle)
#
#     def update_result(optimal_cost, trace_idx):
#         if optimal_cost < target_num[trace_idx]:
#             print("update optimal cost: ", optimal_cost, trace_idx)
#             target_num[trace_idx] = optimal_cost
#             # print("after update", target_num)
#             broadcast_result(optimal_cost, trace_idx)
#
#     def get_latest_result(self):
#         return target_num
#
#     def broadcast_result(optimal_cost, trace_idx):
#         '''
#         broadcast the optimal cost for trace to every worker
#         Parameters
#         ----------
#         optimal_cost
#         trace_idx
...
#             worker.receive_update(optimal_cost, trace_idx)
...
def apply(trace: Trace, petri_net: PetriNet, initial_marking: Marking, final_marking: Marking,
        ...
        alignment = apply_trace_net( petri_net, initial_marking, final_marking, trace_net, trace_im,
                                         trace_fm,
                                         parameters)
...
def apply_from_variants_list(var_list, petri_net, initial_marking, final_marking, parameters=None):
    ...
    -------------
    var_list
        List of variants (for each item, the first entry is the variant itthe second entry may be the number of cases)
    petri_net
        Petri net
    initial_marking
        Initial marking
    ...
    while not len(open_set) == 0:
        ...
        if curr.h < 0.01:
            if current_marking == fin:
                ...
                # print("find optimal alignment for trace: ",  "with cost: ", optimal_alignments['cost'],
                #       " and time: ", time.time() - start_time)
                # # Update coordinator with the local result
                # ray.get(coordinator.update_result.remote(optimal_cost, trace_idx))
                print("optimal alignment: ", optimal_alignments['cost'])
...
def compute_alignment(sub_var):
    ...
    print("net transition num: ", len(net.transitions))
    ...
    optimal_cost_lst = [10000000 for i in range(len(event_log))]
    # iterate every case in this xes log file
    for case_index in range(len(event_log)):
        apply(event_log[case_index], net, im, fm)
...
# get the petri list
log_path = "../log/prAm6_variants_10.xes"
event_log = xes_importer.apply(log_path)
log_len = len(event_log)
model_path1 = '../model/prAm6_id_sub1.pnml'
model_path2 = '../model/prAm6_id_sub2.pnml'
model_path3 = '../model/prAm6_id_sub3.pnml'
model_path4 = '../model/prAm6_id_sub4.pnml'

model_net1, model_im1, model_fm1 = pnml_importer.apply(model_path1)
model_net2, model_im2, model_fm2 = pnml_importer.apply(model_path2)
model_net3, model_im3, model_fm3 = pnml_importer.apply(model_path3)
model_net4, model_im4, model_fm4 = pnml_importer.apply(model_path4)
...
results = [compute_alignment.remote(sub_task) for sub_task in sub_tasks]
