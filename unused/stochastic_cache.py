...
class AlignmentWithCacheReopenAstar:
    ...
    def __init__(self, ini, fin, cost_function, incidence_matrix, trans2weight, trace_sync, trace_log):
        self.ini = ini
        self.fin = fin
    ...
    def search(self):
        ...
        closed = {}
        inc_matrix = self.incidence_matrix.a_matrix  # the incidence matrix
        cons_matrix = self.incidence_matrix.b_matrix  # the consumption matrix
        ...
        # add initial marking to open set, with a probability  of 1
    ...
    def _expand_from_current_marking(self, curr, cost_vec, closed):
        ...
        for t, cost in trans_to_visit_with_cost:
            ...
            if new_m in closed:
                ...
                if self.split_open_set_flag:
                    ...
                    # explore marking for the first time, namely, neither in open set or infeasible set
                        ...
                        if trustable:
                            trans_probability = 1
                            if not is_log_move(t, '>>'):
                                trans_probability = self.trans2weight[t.name[1]] / transition_prob_sum

                            tp = CacheReopenMarking(new_g + new_h, new_g, new_h, new_m, curr, t, new_x, trustable,
                                                    curr.probability * trans_probability, new_parikh_vec_lst,
                ...
                # put all markings into open set, regardless of the feasiblity of markings
                else:
                    if m_in_open_flag:
                        start_time = timeit.default_timer()
                        marking_to_explore = self.open_set.heap_get(new_m)
    ...
    def _reconstruct_alignment(self, state, ret_tuple_as_trans_desc=False):
        alignment = list()
        path = list()
        print(len(state.parikh_vec_lst))
...
def compute_alignment(xes_file, pnml_file):
    ...
    for case_index in tqdm(range(len(event_log))):
        ...
        if trace_str not in trace_variant_lst:
            ...
            start_time = timeit.default_timer()
            ali_with_regular_astar = AlignmentWithCacheReopenAstar(initial_marking, final_marking, cost_function,
                                                                   incidence_matrix, trans2weight,
    ...
    # df.to_csv('../results/log=' + log_name + '&model=' + model_name + '&algorithm=stochastic_cache' + '.csv', index=False)


if __name__ == '__main__':
    log_path = "../log/road_variants.xes"
    model_path = '../model/road_id02_uemsc.pnml'
